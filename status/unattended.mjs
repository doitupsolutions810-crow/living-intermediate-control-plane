#!/usr/bin/env node
/**
 * Unattended loop: daily → self-develop → admit-change → final gates
 */
...
const skipAdmit = process.env.UNATTENDED_SKIP_ADMIT === '1';
...
let go = push(run('init', 'status/init.mjs'));
if (!skipDaily && go) {
  go = push(run('daily', 'status/daily-loop.mjs', { ALLOW_SKIP: process.env.ALLOW_SKIP || '1' })) || continueOnFail;
}
...
if (!skipAdmit && (go || continueOnFail)) {
  go = push(run('admit-change', 'status/admit-change.mjs', {
    ALLOW_SKIP: process.env.ALLOW_SKIP || '1'
  })) || continueOnFail;
}
...
const summary = {
  ...
  mode: 'unattended',
  note: failed === 0
    ? 'Unattended loop completed under plane gates (incl. admit-change).'
    : 'Unattended loop finished with failures.'
};
